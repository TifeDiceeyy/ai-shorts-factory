...
def test_regenerate_scene_reuses_untouched_scenes_on_disk(tmp_path):
    ...
    # exactly 1 new TTS entry; it adds a 2nd (image) entry only if scene 0 is
    # an ingredient_grid/process_action type — mascot-type scenes reuse the
    # hero image already generated by the prior full run, at zero extra cost
    # (see pipeline._get_or_create_hero_image).
    regen_scene_type = script["scenes"][0].get("scene_type")
    expected_new_entries = 2 if regen_scene_type in ("ingredient_grid", "process_action") else 1
    assert len(result.cost_report["entries"]) == prior_cost_entries + expected_new_entries
